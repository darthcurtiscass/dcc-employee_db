...
//query to see all Department table data
function getDepartment() {
  return db.promise().query("SELECT * FROM department;");
}
//query to see all Manager table data
function getManagers() {
  return db
    .promise()
    .query("SELECT first_name, last_name FROM employee WHERE manager_id;");
}
//query to see all Roles table data
function getRoles() {
  return db.promise().query("SELECT * FROM roles;");
}
//query to see all Employee table data
...
function addEmployee() {
  ...
  inquirer
    ...
    .then((data) => {
      ...
      db.query(
        `SELECT (SELECT id FROM roles WHERE roles.title = '${data.employeeRole}') role_id,
            (SELECT id FROM employee WHERE first_name = '${managerName[0]}' AND last_name = '${managerName[1]}') manager_id;`,
        function (err, results) {
          if (err) throw err;

          console.log("Captured ids successfully");
          db.query(
            `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ('${data.firstName}', '${data.lastName}', ${results[0].role_id}, ${results[0].manager_id});`,
            function (err, results) {
              if (err) throw err;
...
//create array for department choices, query to add new role
function addRole() {
  ...
  inquirer
    ...
    .then((data) => {
      db.query(
        `SELECT id department_id FROM department WHERE department.department_name = '${data.roleDepartment}';`,
        function (err, results) {
          if (err) throw err;

          db.query(
            `INSERT INTO roles (title, salary, department_id) VALUES ('${data.title}', ${data.salary}, ${results[0].department_id});`,
            function (err, results) {
              if (err) throw err;
              console.log("Insert successful");
              option();
...
//use to view all departments in the terminal.
...
//o view all Roles in the terminal.
...
//updates the role of a specific employee. First by retreiving the data from the employee and roles tables respectively,
// then the data is pushed as an array to the inquirer prompt.
async function updateEmployeeRole() {
  employeeNames = [];
  const employeeArray = await getEmployees();
  for (var i = 0; i < employeeArray[0].length; i++) {
    const emp =
      employeeArray[0][i].first_name + " " + employeeArray[0][i].last_name;
  ...
  inquirer
    ...
    .then((data) => {
      ...
      db.query(
        `SELECT (SELECT id FROM roles WHERE roles.title = '${data.newRoleChoice}') role_id,
            (SELECT id FROM employee WHERE first_name = '${empUpdate[0]}' AND last_name = '${empUpdate[1]}') emp_id;`,
        function (err, results) {
          if (err) throw err;
          console.log(results[0]);

          db.query(
            `UPDATE employee SET role_id = ${results[0].role_id} WHERE role_id = ${results[0].emp_id};`,
            function (err, results) {
              if (err) throw err;
